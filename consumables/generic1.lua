SMODS.Consumable {
    ...
    loc_txt = {
        name = 'Hydrogen',
        text = {
        [1] = 'Level up {C:attention}High Card{} by the number',
    ...
    use = function(self, card, area, copier)
        local used_card = copier or card
            ...
            update_hand_text({ sound = 'button', volume = 0.7, pitch = 0.9, delay = 0 }, { level = '+'..tostring(G.GAME.hands['High Card'].played) })
            delay(1.3)
            level_up_hand(card, "High Card", true, G.GAME.hands['High Card'].played)
